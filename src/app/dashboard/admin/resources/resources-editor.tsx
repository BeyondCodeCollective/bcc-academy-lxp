          ...
          <div key={i} className="panel space-y-3 p-4">
            <div className="flex items-start gap-3">
              <div className="grid flex-1 grid-cols-1 gap-3 sm:grid-cols-[1fr_1fr_4rem]">
                ...
                <label className="block">
                  <span className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-ink-faint">Category</span>
                  <input type="text" value={r.category} onChange={(e) => update(i, "category", e.target.value)} placeholder="Tools" className={fieldInput} />
                </label>
                <label className="block">
                  <span className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-ink-faint">Icon</span>
              ...
              <button type="button" onClick={() => remove(i)} aria-label="Remove" className="mt-5 shrink-0 rounded-md p-1.5 text-ink-faint transition-colors hover:bg-paper-tint hover:text-ink">
                <Trash2 size={15} aria-hidden />
              </button>
            </div>
            <label className="block">
              <span className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-ink-faint">Link (optional)</span>
              <input type="text" value={r.url} onChange={(e) => update(i, "url", e.target.value)} placeholder="https://…" className={fieldInput} />
            </label>
            <label className="block">
              <span className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-ink-faint">Description (optional)</span>
