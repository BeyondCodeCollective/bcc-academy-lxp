...
/**
 * Signed out, the help center is a prospectus, not a manual: what the platform
 * is, how a learner uses it, and what we do with their data. Everything else —
 * how to take attendance, how cohorts and gating work, how roles are shaped —
 * describes running the place, and reads as a map of it to anyone who asks.
 */
const PUBLIC_SECTIONS = ["getting-started", "for-students"];
const PUBLIC_ARTICLES = ["data-privacy"];

function visibleSections(signedIn: boolean): Section[] {
  if (signedIn) return SECTIONS;
  return SECTIONS.map((section) =>
    PUBLIC_SECTIONS.includes(section.id)
      ? section
      : {
          ...section,
          articles: section.articles.filter((a) => PUBLIC_ARTICLES.includes(a.id)),
        },
...
export function HelpCenter({ signedIn }: { signedIn: boolean }) {
  const sections = useMemo(() => visibleSections(signedIn), [signedIn]);
    ...
    <div className="min-h-screen bg-paper text-ink">
      ...
      <header className="mx-auto max-w-7xl px-6 pt-16">
        <div className="flex items-start justify-between gap-6">
          ...
          <Link
            href={signedIn ? "/dashboard" : "/login?next=%2Fhelp"}
      ...
      {!activeArticleData && (
        <div className="mx-auto max-w-7xl px-6 py-10">
          ...
          {/* Say what is behind the sign-in rather than quietly showing a
              shorter list — a visitor who came looking for the instructor
              guide should know where it went. */}
          {!signedIn && (
            <div className="panel mt-8 flex flex-wrap items-center justify-between gap-4 p-5">
              <p className="text-sm text-ink-soft">
                The instructor, admin, course and platform guides are for enrolled
                learners and staff.{" "}
                <span className="text-ink">Sign in to read them.</span>
              </p>
              <Link
                href="/login?next=%2Fhelp"
                className="shrink-0 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-primary-hover"
              >
                Sign in
              </Link>
            </div>
